...
class Money is export {
    ...
    multi method new(Str $input) {
        if $input ~~ /<money-regex>/ {
            self.bless(amount => $<money-regex><money-amount>.FatRat, currency => Currency.new($<money-regex><currency-code>.Str));
        } else {
            fail { "Not a valid money string input" };
        }
    }

    multi method new(FatRat :$amount, Currency :$currency) {
    ...
    multi method new(Rat :$amount, Str :$currency) {
        self.bless(:amount($amount.FatRat), :currency(Currency.new($currency)))
    }
